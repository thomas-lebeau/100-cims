...
export function useCimsQuery(includesComarca: true): Response & { data: CimWithComarca[] };
export function useCimsQuery(includesComarca: false): Response & { data: Cim[] };
export function useCimsQuery(includesComarca: boolean): Response & { data: Cim[] | CimWithComarca[] };
export function useCimsQuery(includesComarca: boolean = false) {
  const { data, error, isFetching } = useQuery({
    queryKey: ["cims", includesComarca],
    queryFn: () => {
      if (includesComarca) {
        return zfetch(cimsWithComarcaSchema.array(), "/api/cims?includeComarcas=true");
      }
